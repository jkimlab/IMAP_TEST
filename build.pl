...
if($check){
	...
	`rm -rf *.log bwa-0.7.17 GapCloser kent lastz-distrib-1.04.00 DESCHRAMBLER RACA-0.9.1.1 SOAPdenovo2-bin-LINUX-generic-r240 SPAdes-3.7.1-Linux MaSuRCA-3.3.1 samtools-0.1.18`;
	chdir("$Bin");
	`rm -f $Bin/path.conf`;
} elsif($install){
...
#### Compile samtools
`tar xf $source_dir/samtools-0.1.18.tar.bz2 -C $thirdparty_path`;
`make -C $thirdparty_path/samtools-0.1.18 2> $thirdparty_path/samtools.log`;
if(-f "$thirdparty_path/samtools-0.1.18/samtools"){
	print STDERR "Done\n";
} else {
	print STDERR "ERROR\n";
	exit(1);
...
print W "samtools=$thirdparty_path/samtools-0.1.18/samtools\n";
