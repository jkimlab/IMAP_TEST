...
help () {
	...
	echo "		-s PATH path of 'snp_edit' script"
	exit;
}
...
for idx in ${!P1[@]};do
	...
	$samtools view -h -bS $OUTPUT_BWA/$res.sam 1> $OUTPUT_BWA/$res.bam 2>> $OUTPUT_BWA/log
...
java -jar $pilon --genome $INPUT_FASTA --frags $OUTPUT_MARKDUP/$res --output $OUTPUT_PILON/$(basename $INPUT_FASTA .fa).pilon 2> $OUTPUT_PILON/log
...
$snpedit $OUTPUT_VARIANT/$vcf $OUTPUT_FASPLIT $OUTPUT_FINAL/$res 2> $OUTPUT_FINAL/log
